...
void PairHMM_align_partition_half_allowMultipleBatchesPerWarp(
    ...
    if(alignmentId < numAlignments){
        ...
        //             alignmentId, myGroupMask, batchId, groupIdInBatch, hapToProcessInBatch, readIndexToProcessInBatch, readToProcessInBatch, readLength, haploLength, numAlignments);
        //     }
        // }
...
template <int group_size, int numRegs> 
__global__
void PairHMM_align_partition_half_allowMultipleBatchesPerWarp_chunked(
    const uint8_t * read_chars,
    const uint8_t * hap_chars,
    const uint8_t * base_quals,
    const uint8_t * ins_quals,
...
std::vector<float> processBatch_overlapped(const batch& fullBatch_default, const Options& options){
    ...
    for(int computeChunk = 0, transferChunk = 0; computeChunk < numTransferChunks; computeChunk++){
        ...
        #define COMPUTE_NUM_ALIGNMENTS_AND_PAIRHMM(stream) \
            ...
            PairHMM_align_partition_half_allowMultipleBatchesPerWarp_chunked<group_size,numRegs><<<numBlocks, blocksize,0,stream>>>( \
                dev_read_chars,  \
                dev_hap_chars,  \
                dev_base_qual,  \
                dev_ins_qual,  \
...
int main(const int argc, char const * const argv[])
{
    ...
    //             cout << " " << resultsBatchAsWhole[res_off+j*fullBatch.batch_haps[i]+k];
    //     cout << " \n";
